...
class PlayerGuiCtk(customtkinter.CTk):
    ...
    def validate_height_input(self, value):
        return value.isdigit() or (value.endswith('cm') and value[:-2].isdigit()) or (value == "" and self.height_entry.get().isdigit())


    def validate_jersey_input(self, value):
        return value.isdigit() or (value == "" and self.jersey_entry.get().isdigit())
 ...
 # Styling enhancements
        ...
        self.id_entry = self.newCtkEntry(entryiable=self.id_entry, validate='key', validatecommand=(self.register(self.validate_id_input), '%P'))
        ...
        self.height_entry = self.newCtkEntry(entryiable=self.height_entry, validate='key', validatecommand=(self.register(self.validate_height_input), '%P'))
        ...
        self.jersey_entry = self.newCtkEntry(entryiable=self.jersey_entry, validate='key', validatecommand=(self.register(self.validate_jersey_input), '%P'))
    ...
    # new Entry Widget
    # new Entry Widget
    def newCtkEntry(self, text='CTK Label', entryiable=None, validate=None, validatecommand=None):
        widget_Font = self.font1
        ...
        widget = customtkinter.CTkEntry(self,
                                    ...
                                    width=widget_Width,
                                    validate=validate,  # Add validate argument
                                    validatecommand=validatecommand)  # Add validatecommand argument
    ...
    def read_display_data(self, event):
        ...
        # Check if row[3] is an integer (jersey number) or a string (height)
            if isinstance(row[3], int):
                self.height_entry.delete(0, END)  # Clear the height entry
            else:
                self.height_entry.insert(0, row[3].split(' ')[0])  # Display height in the entry
    ...
    def add_entry(self):
        ...
        height = f"{self.height_entry.get()} cm" if self.height_entry.get() else ""
